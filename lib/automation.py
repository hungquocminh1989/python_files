...
class FacebookInstance:
    def __init__(self, username, password, remote_url=''):
        ...
        self.init_web_url()
        self.dblogs.debug_log(f'{self.user_debug_const}Object created')
    ...
    def newfeeds(self):
        ...
        self.dblogs.debug_log(f'{self.user_debug_const}Redirect to newfeeds')

    def profile(self):
        self.browser.action_redirect(self.profile_url)
        self.dblogs.debug_log(f'{self.user_debug_const}Redirect to profile')
    ...
    def login(self):
        ...
        if is_login == False:
            ...
            self.dblogs.debug_log(f'{self.user_debug_const}Redirect to login : {self.login_url}')
            
            self.browser.action_input_text('//*[@id="m_login_email"]', self.username)
            self.dblogs.debug_log(f'{self.user_debug_const}Input username')
            
            self.browser.action_input_text('//*[@id="m_login_password"]', self.password)
            self.dblogs.debug_log(f'{self.user_debug_const}Input password')
            
            self.browser.action_input_click('//*[@id="u_0_4"]/button')
            self.dblogs.debug_log(f'{self.user_debug_const}Click login button')
    ...
    def check_login_status(self):
        ...
        self.dblogs.debug_log(f'{self.user_debug_const}Check login status')
        if exist == False:
            return True
    ...
    def post_to_page(self, page_url, content, arr_images):
        ...
        self.dblogs.debug_log(f'{self.user_debug_const}Redirect to : {page_url}')
        
        self.browser.action_input_click('//*[@id="action_bar"]/div[1]/a')
        self.dblogs.debug_log(f'{self.user_debug_const}Click button create new post')
        
        self.browser.action_input_text('//*[@id="u_0_1h"]', content)
        self.dblogs.debug_log(f'{self.user_debug_const}Input post content')
        
        for image in arr_images:
            self.browser.action_autoit_upload_file('//*[@id="structured_composer_form"]/div[5]/div/div[1]/button[1]',image)
            self.dblogs.debug_log(f'{self.user_debug_const}Select and upload file image : {image}')
            
        #self.browser.action_input_click('//*[@id="composer-main-view-id"]/div[1]/div/div[3]/div/button[1]')
        #self.dblogs.debug_log(f'{self.user_debug_const}Click button post')
