...
class FacebookInstance:
    def __init__(self, username, password, dynamic_user_data=False):
        self.autoload = common.AutoLoad()
        self.dynamic_user_data = dynamic_user_data
        self.username = username
        self.user_debug_const = f'[{username} DEBUG] - '
        self.password = password
        self.init_web_instance()
        self.cloudinary = common.Cloudinary(cloud_name=self.autoload.config['CLOUDINARY']['CLOUD_NAME'], api_key=self.autoload.config['CLOUDINARY']['API_KEY'], api_secret=self.autoload.config['CLOUDINARY']['API_SECRET'])

    def init_web_instance(self):
        self.browser = common.SeleniumInstance(headless=True, session=f'Facebook_{self.username}', auto_detect_timeout=True, dynamic_user_data=self.dynamic_user_data)
        self.browser.enable_auto_screenshot()
        self.browser.dblogs.debug_log(f'{self.user_debug_const}Object created')


    def newfeeds(self):
        self.browser.action_redirect(self.autoload.config['FACEBOOK_URL']['HOME_URL'])
        self.browser.dblogs.debug_log(f'{self.user_debug_const}Redirect to newfeeds')

    def profile(self):
        self.browser.action_redirect(self.autoload.config['FACEBOOK_URL']['PROFILE_URL'])
    ...
    def login(self):
        ...
        if is_login == False:
            self.browser.action_redirect(self.autoload.config['FACEBOOK_URL']['LOGIN_URL'])
            self.browser.dblogs.debug_log(f'{self.user_debug_const}Redirect to login : {self.login_url}')
            
            self.browser.action_input_text(self.autoload.config['FACEBOOK_XPATH']['EMAIL'], self.username)
            self.browser.dblogs.debug_log(f'{self.user_debug_const}Input username')
            
            self.browser.action_input_text(self.autoload.config['FACEBOOK_XPATH']['PASSWORD'], self.password)
            self.browser.dblogs.debug_log(f'{self.user_debug_const}Input password')
            
            self.browser.action_input_click(self.autoload.config['FACEBOOK_XPATH']['BUTTON_LOGIN'])
    ...
    def post_to_page(self, page_url, content, arr_images):
        ...
        self.browser.action_input_click(self.autoload.config['FACEBOOK_XPATH']['BUTTON_CREATE_POST'])
        self.browser.dblogs.debug_log(f'{self.user_debug_const}Click button create new post')
        
        self.browser.action_input_text(self.autoload.config['FACEBOOK_XPATH']['TEXTBOX_CONTENT_POST'], content)
        self.browser.dblogs.debug_log(f'{self.user_debug_const}Input post content')
        
        for image in arr_images:
            self.browser.action_autoit_upload_file(self.autoload.config['FACEBOOK_XPATH']['BUTTON_UPLOAD_IMAGE_POST'],self.browser.action_download_file(image))
            self.browser.dblogs.debug_log(f'{self.user_debug_const}Select and upload file image : {image}')
            
        self.browser.action_input_click(self.autoload.config['FACEBOOK_XPATH']['BUTTON_PUBLISH_POST'])
